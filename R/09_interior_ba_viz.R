...
#   Right : 2D density rainbow (viridis) showing the mass distribution of (true BAF, diff).
...
# interior: exclude the near-boundary psi=-0.01 level (highest true BAF; the ceiling-effect driver)
sub <- df[df$psi != -0.01, ]
...
# regression of diff on true BAF -> significance of proportional bias & mean bias
...
p_scatter <- ggplot(ss, aes(x = bf_true, y = y, color = zone)) +
  # wider width -> true-BAF "columns" become continuous clouds, no more visible vertical bars
  ...
  labs(title = "A. Jittered scatter (horizontal spread)",
       subtitle = "X jittered within each true-BAF level; no per-condition mean dot",
       x = "True BAF (interior, excludes near-boundary psi=-0.01)",
...
cap_09 <- sprintf(
  "Regression (diff ~ true BAF, interior): slope = %.3f (t = %.1f, p %s); intercept = %.3f (t = %.1f, p %s). Slope p < 0.05 => significant proportional bias.",
