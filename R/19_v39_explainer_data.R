# R/19_v39_explainer_data.R
# ---------------------------------------------------------------------------
# Chart-level data export for the v39 interactive explainer.
#
# R/18_v39_three_part_analysis.R produces the scalar single-source-of-truth
# (output/tables/v39_all_numbers.json). This script adds the *series* data the
...
#
# Output: output/tables/v39_explainer_data.json
...
#   $roc         ROC curves (thinned) for calibrated p, BAF, BAF + CI width
...
# ---------------------------------------------------------------------------
cat("[5] BAF_hat quantiles by BAF_true ...\n")
...
# ---------------------------------------------------------------------------
write_json(EX, file.path(TAB_DIR, "v39_explainer_data.json"),
           auto_unbox = TRUE, digits = 6, na = "null")
cat(sprintf("[done] wrote %s (%.0f KB)\n",
            file.path(TAB_DIR, "v39_explainer_data.json"),
            file.size(file.path(TAB_DIR, "v39_explainer_data.json")) / 1024))
