# R/18_v38_three_part_analysis.R
# ---------------------------------------------------------------------------
# Master analysis script for manuscript v38 ("第38版").
#
# The v38 paper is organised in three sequential parts. This script produces
# every number the manuscript quotes for Parts I-III, from the existing
# v38b (2026-09-01): the negative-control panel size K gained a third level
...
#
#   Part I   The metric. BF = |mu_B_hat| / (|mu_B_hat| + psi_tilde).
...
#            read through the calibrated p + BF + CI-width interface.
#
# Outputs
#   output/tables/v38_part1_bland_altman.csv
#   output/tables/v38_part2_reliability_lookup.csv
#   output/tables/v38_part2_strategy_comparison.csv
#   output/tables/v38_part2_confusion3x3.csv
#   output/tables/v38_part2_subgroups.csv
#   output/tables/v38_part2_discrimination.csv
#   output/tables/v38_part3_case_verdicts.csv
#   output/tables/v38_all_numbers.json          <- single source of truth
#   output/figures/v38/figP1_*.png  figP2_*.png
# ---------------------------------------------------------------------------
...
# ===========================================================================
# PART II - Step 1.  Bland-Altman agreement of the continuous BF estimator
# ===========================================================================
...
write.csv(BA, file.path(TAB_DIR, "v38_part1_bland_altman.csv"), row.names = FALSE)
...
write.csv(LOOK, file.path(TAB_DIR, "v38_part2_reliability_lookup.csv"), row.names = FALSE)
...
DISC <- data.frame(
  predictor = c("calibrated p-value alone", "BF_hat alone",
                "BF_hat + CI half-width", "BF_hat + CI half-width + calibrated p"),
...
write.csv(DISC, file.path(TAB_DIR, "v38_part2_discrimination.csv"), row.names = FALSE)
print(DISC, digits = 4)
...
write.csv(CIRULE, file.path(TAB_DIR, "v38_part2_ci_width_rules.csv"), row.names = FALSE)
...
strat <- list(
  ...
  `R2 R1 + BF point estimate < 0.5`              = L1 & S$bf < BF_THRESH_BIAS,
  `R3 R1 + BF 95% CI contained in one zone`      = L1 & S$zone == "effect-dominated",
  ...
  `C1 BF point estimate alone`                   = S$bf < BF_THRESH_BIAS,
  `C2 BF 95% CI contained in one zone, alone`    = S$zone == "effect-dominated"
)
...
write.csv(STRAT, file.path(TAB_DIR, "v38_part2_strategy_comparison.csv"), row.names = FALSE)
...
write.csv(flow, file.path(TAB_DIR, "v38_part2_flow.csv"), row.names = FALSE)
print(flow, digits = 3)
NUM$part2$flow <- flow

## 2.7 3x3 confusion matrix (truth zone x BF zone) --------------------------
...
write.csv(cbind(truth = rownames(CM), CM),
          file.path(TAB_DIR, "v38_part2_confusion3x3.csv"), row.names = FALSE)
...
write.csv(SUB, file.path(TAB_DIR, "v38_part2_subgroups.csv"), row.names = FALSE)
print(SUB, digits = 3)
...
write.csv(CASE, file.path(TAB_DIR, "v38_part3_case_verdicts.csv"), row.names = FALSE)
...
gA <- ggplot(rl, aes(bf, p, colour = cls, shape = cls)) +
  ...
  labs(x = expression(hat(BF)~"reading (bin median)"),
       y = "P(truth is bias-dominated)",
       colour = NULL, shape = NULL,
       title = "Layer 2 calibration of the continuous BF reading",
...
gB <- ggplot(fl2, aes(stage, n, fill = truth)) +
  ...
  labs(x = NULL, y = "Simulated repetitions", fill = NULL,
       ...
       caption = "Layer 1 keeps repetitions with calibrated p < .05; layer 2 grades them by the BF reading and its CI width.") +
...
rc <- rbind(
  ...
  cbind(roc_pts(S$bf, S$bd_tru), model = "BF_hat alone"),
  cbind(roc_pts(predict(m_bfci, type = "link"), S$bd_tru), model = "BF_hat + CI half-width"))
gC <- ggplot(rc, aes(fpr, tpr, colour = model)) +
  ...
  labs(x = "False-positive rate", y = "True-positive rate", colour = NULL,
       ...
       caption = sprintf("All %s repetitions (the full %d-condition x %d-repetition design). AUC: calibrated p %.3f; BF_hat %.3f; BF_hat + CI width %.3f.",
                         format(nrow(S), big.mark = ","),
                         length(res),
                         nrow(S) %/% length(res),
...
gD <- ggplot(S, aes(half, fill = factor(K))) +
  ...
  labs(x = "95% CI half-width of BF_hat", y = "Repetitions",
...
gP1 <- ggplot(ci_ba, aes(bf_true, diff)) +
  ...
  labs(x = "True BF", y = expression(hat(BF) - BF[true]),
       title = "Step 1: agreement of the BF estimator (interior analysis)",
...
# ===========================================================================
write_json(NUM, file.path(TAB_DIR, "v38_all_numbers.json"),
           auto_unbox = TRUE, digits = 8, pretty = TRUE, na = "null")
cat("[done] wrote v38_all_numbers.json and 8 CSV tables; figures in output/figures/v38/\n")
