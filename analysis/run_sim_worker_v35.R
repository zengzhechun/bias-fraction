# Credibility Metric Simulation Study — No-bootstrap worker (v35, BF scale)
# Usage: Rscript run_sim_worker_v35.R <ex_violation> <n_nc>
...
# v35 fix (review M6/G1): null conditions (true_log_rr = 0) now retain
#   bf_true = 1 (finite) instead of NA, so BF bias/RMSE statistics include the
#   null — the very scenario where BF stays finite while BER diverges. The fix
#   consumes no extra random draws, so all estimates are bit-identical to v34.
...
log_file <- file.path(LOG_DIR, sprintf("sim_v35_%s.log", worker_id))
...
# NOTE on the exchangeability-violation proportion: the shifted controls form a
# fixed, pre-specified subset occupying the last round(n_nc * ex_violation)
# positions of the bias vector (4/12 = 33.3% for K=12 and 8/25 = 32.0% for K=25
# at the 0.3 level). Distributionally equivalent to a random 30% subset; the
# deterministic implementation is documented in the Supplement (review M7/C2).
sim_grid$bsr_true <- ifelse(abs(sim_grid$true_log_rr) < 1e-10, Inf,
  abs(sim_grid$bias_mu) / abs(sim_grid$true_log_rr))
# v35: bf_true is finite everywhere — 1 at the null (review M6/G1), ber_to_bf()
# elsewhere (ber_to_bf(Inf) would return NaN, which v34 stored and then dropped).
sim_grid$bf_true  <- ifelse(is.finite(sim_grid$bsr_true), ber_to_bf(sim_grid$bsr_true), 1)
sim_grid$n_nc <- n_nc
...
run_one_condition <- function(true_log_rr, bias_mu, bias_sigma, n_nc,
  ...
  # v35: bf_true = 1 at the null (finite), so BF bias/RMSE include null conditions
  bf_true  <- if (is.finite(bsr_true)) ber_to_bf(bsr_true) else 1
  ...
  # v35: bf_true is now finite at the null (= 1), so bf_rmse / bf_rel_bias are
  # computed for ALL conditions, including true_log_rr = 0 (review M6/G1).
  ...
  data.frame(
    ...
    bf_rmse  = sqrt(mean((bf_est[ok] - bf_true)^2, na.rm = TRUE)),
    bf_rel_bias = (mean(bf_est[ok], na.rm = TRUE) - bf_true) / bf_true,
...
cat("\n===== Summary (v35, BF scale) =====\n", file = log_con)
finite_idx <- is.finite(sim_df$bsr_true)
null_idx <- sim_df$true_log_rr == 0
cat(sprintf("BF RMSE (mean, non-null): %.4f\n", mean(sim_df$bf_rmse[finite_idx], na.rm = TRUE)), file = log_con)
cat(sprintf("BF RMSE (median, non-null): %.4f\n", median(sim_df$bf_rmse[finite_idx], na.rm = TRUE)), file = log_con)
cat(sprintf("BF RMSE (mean, all incl. null): %.4f\n", mean(sim_df$bf_rmse, na.rm = TRUE)), file = log_con)
cat(sprintf("BF relative bias (mean, non-null): %.1f%%\n",
  100 * mean(sim_df$bf_rel_bias[finite_idx], na.rm = TRUE)), file = log_con)
cat(sprintf("BF relative bias (null, bf_true=1): %.1f%%\n",
  100 * mean(sim_df$bf_rel_bias[null_idx], na.rm = TRUE)), file = log_con)
cat(sprintf("BF RMSE (null): %.4f\n", mean(sim_df$bf_rmse[null_idx], na.rm = TRUE)), file = log_con)
...
out_file <- file.path(SIM_DIR, sprintf("sim_results_v35_%s.rds", worker_id))
